...
async function testGetFileUrls() {
    try {
        ...
        const response = await apiClient.get('/lawyer/profile/files/urls?fileTypes=photo,cv,bar_registration_certificate');

        console.log('✅ Pre-signed URLs generated successfully');
        console.log('📄 Response:', JSON.stringify(response.data, null, 2));

        // Test accessing a file using the pre-signed URL
        if (response.data.urls && response.data.urls.photo) {
            console.log('\n🔍 Testing file access via pre-signed URL...');
            try {
                const fileResponse = await axios.get(response.data.urls.photo, {
...
async function testErrorScenarios() {
    try {
        ...
        // Test profile update without required fields
        const formData = new FormData();
        formData.append('full_name', 'Test Error');
        ...
        // Test getting URLs for non-existent files
        try {
            await apiClient.get('/lawyer/profile/files/urls?fileTypes=nonexistent');
        } catch (error) {
            console.log('✅ Invalid file type error handled correctly:', error.response?.data?.message || 'No error returned');
        }
        ...
        try {
            ...
            console.log('✅ Non-existent file deletion error handled correctly:', error.response?.data?.message);
        }
